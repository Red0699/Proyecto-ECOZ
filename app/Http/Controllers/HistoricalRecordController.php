...
class HistoricalRecordController extends Controller
{
    public function index(Request $request)
    {
        ...
        $series = [
            ...
            'cov_kg'        => [],
            'co2_kg'        => [],
            // NUEVAS series para la gráfica de variación de galonaje
            'var_evap_gl'   => [], // sumatoria_variacion_gl
            'var_total_gl'  => [], // variacion_eds_gl
        ];
        $kpis = [
            'cov_total_kg'          => 0,
            'co2_total_kg'          => 0,
        ...
        // Por defecto: último mes/año con datos para que el SELECT muestre el correcto
        if (!$request->has('month')) {
            $month = (int) ($lastFecha ? $lastFecha->format('m') : now()->format('m'));
        }
        if (!$request->has('year')) {
            $year  = (int) ($lastFecha ? $lastFecha->format('Y') : now()->format('Y'));
        }
        ...
        if ($mode === 'year') {
            ...
            $grouped = $base->selectRaw("
                ...
                SUM(sumatoria_variacion_gl)             AS var_evap_gl,
                SUM(variacion_eds_gl)                   AS var_total_gl
        ...
        $series['var_evap_gl']   = $grouped->pluck('var_evap_gl')->map(fn($v) => (float)$v)->all();
        $series['var_total_gl']  = $grouped->pluck('var_total_gl')->map(fn($v) => (float)$v)->all();
